...
| **n8n + 11 workflows** | le cerveau (lit, écrit, rédige, route, source, tracke, signe, encaisse, alerte) | ta machine (`localhost:5678`) |
...
## Phase 2 — Le cerveau : n8n local + les 11 workflows (25 min)
...
### 2.3 Importer les 11 workflows (dans cet ordre)
...
| 6 | `alpha-deepdive.workflow.json` | **Ollama** + Sheets (`SHEET_ID`) — recherche externe collée → audit structuré dans le CRM | **Activer** → formulaire sur `http://localhost:5678/form/alpha-deepdive` |
| 7 | `alpha-sourcing.workflow.json` | Sheets ×2 (`SHEET_ID`) — Places/Pappers/Apollo lisent `GOOGLE_PLACES_KEY`/`PAPPERS_TOKEN`/`APOLLO_API_KEY` | **Activer** → formulaire sur `http://localhost:5678/form/alpha-sourcing` |
| 8 | `alpha-tracking-sync.workflow.json` | Supabase + Sheets (`SHEET_ID`) — écrit délivré/ouvertures/clics dans le Sheet (indispensable si tracking sur Vercel + n8n local) | **Activer** |
| 9 | `alpha-error-alert.workflow.json` | Gmail — le destinataire lit `ALPHA_ALERT_EMAIL` | **Save** (pas besoin d'activer) |
| 10 | `alpha-signature.workflow.json` *(optionnel)* | rien à mapper — lit `ALPHA_DOCUMENSO_SECRET`/`ALPHA_CRM_*` ; côté Documenso : Settings → Webhooks | **Activer** quand tu utilises Documenso |
| 11 | `alpha-payment.workflow.json` *(optionnel)* | credential **Stripe** (clé secrète) | **Activer** quand tu encaisses via Stripe |

**Puis branche l'alerte** : sur **chacun** des workflows 1 à 8 (et 10–11
quand tu les actives), ouvre le
